"""Local replica of the professor's search-missions webapp.

Mirrors the webapp API so strategies and notebook code can use the same
``run_mission`` call against either the simulator or the real submission.
"""
...
class SearchEnvironment:
    ...
    def plant_object(
        self, cell_id: int | None = None, prior: np.ndarray | None = None
    ) -> int:
        if cell_id is None:
            if prior is None:
        ...
        self.true_cell = int(cell_id)
        return self.true_cell
